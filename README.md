...
- **剪映草稿图片添加器**: 向现有剪映草稿添加图片，支持按顺序排列，每张图片展示固定时长
...
向现有剪映草稿添加图片，支持按顺序排列，每张图片展示固定时长。

#### 输入参数

| 参数名 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| 图片文件路径 | STRING | 空 | 要添加到草稿的图片文件路径 |
| 剪映草稿路径 | STRING | 空 | 剪映草稿文件夹路径，包含draft_meta_info.json和draft_content.json |
| 单张图片时长 | FLOAT | 3.0 | 每张图片展示的时长（秒），范围0.5-30秒 |
| 图片顺序 | 选择列表 | append | append: 添加到现有图片后面, insert_first: 插入到最前面 |
...
3. 输入剪映草稿路径（通常来自第一个节点的输出）
4. 输入图片文件路径
5. 可选：设置单张图片展示时长（默认为3秒）
6. 可选：设置图片顺序（append: 添加到末尾，insert_first: 插入到最前面）
7. 运行节点添加图片
8. 重复步骤2-7添加更多图片，系统会自动按顺序排列
